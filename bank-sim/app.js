...
function goalSheet(index){
  ...
  $('#toGoal').onclick=()=>{const a=num($('#goalMoveAmount').value);if(!a||a<=0){toast('Введите сумму');return}if(a>state.account.balance){toast('Недостаточно на основном счёте');return}state.account.balance-=a;g.balance+=a;state.tx.unshift({title:`В ${g.name}`,amount:-a,sub:'Только что'});render();goalSheet(index);toast('Переведено в накопительный')};
  $('#fromGoal').onclick=()=>{const a=num($('#goalMoveAmount').value);if(!a||a<=0){toast('Введите сумму');return}if(a>g.balance){toast('Недостаточно в накопительном');return}g.balance-=a;state.account.balance+=a;state.tx.unshift({title:`Из ${g.name}`,amount:a,sub:'Только что'});render();goalSheet(index);toast('Переведено на основной')};
...
function amountSheet(kind){
  ...
  openSheet(`<h3>${top?'Пополнить '+safe(state.account.name):'Перевести'}</h3>${top?'':'<div class="field"><label>Получатель</label><input id="recipient" value="Алексей"></div>'}<div class="field"><label>Сумма</label><input id="amount" inputmode="decimal" placeholder="1000"></div><button class="primary" id="confirmAmount">${top?'Пополнить':'Перевести'}</button><div class="note">Это изменение локального баланса симулятора, без настоящей банковской операции.</div>`);
  $('#confirmAmount').onclick=()=>{const a=num($('#amount').value);if(!a||a<=0){toast('Введите сумму');return}if(!top&&a>state.account.balance){toast('Недостаточно средств');return}if(top){state.account.balance+=a;state.tx.unshift({title:'Пополнение',amount:a,sub:'Только что'});toast('Баланс изменён')}else{state.account.balance-=a;state.spend+=a;state.tx.unshift({title:'Перевод',amount:-a,sub:'Только что'});toast('Операция добавлена')}render();closeSheet()}
}
...
function historyHtml(){return state.tx.map(t=>`<div style="display:flex;align-items:center;gap:11px;padding:11px 0;border-bottom:1px solid #303034"><div style="width:38px;height:38px;border-radius:13px;background:#2a2a2d;display:grid;place-items:center">${t.amount>0?'＋':'•'}</div><div style="flex:1"><b style="font-size:14px">${safe(t.title)}</b><div style="color:#8f8f94;font-size:11px;margin-top:2px">${safe(t.sub)}</div></div><strong style="font-size:14px;white-space:nowrap;color:${t.amount>0?'#18bf50':'#fff'}">${t.amount>0?'+':'−'}${rub(t.amount)}</strong></div>`).join('')}
function searchSheet(){
  const items=[{title:'Все операции',sub:'История трат',go:()=>$('#historyTile').click()},{title:'Кэшбэк и бонусы',sub:'Начисления',go:()=>$('#cashbackTile').click()},{title:state.account.name,sub:'Основной счёт',go:editAccountSheet},...state.goals.map((g,i)=>({title:g.name,sub:'Накопительный счёт',go:()=>goalSheet(i)}))];
  ...
  const draw=q=>{const m=items.filter(x=>(x.title+' '+x.sub).toLowerCase().includes(q.toLowerCase()));$('#searchResults').innerHTML=m.map((x,i)=>`<button data-search-i="${items.indexOf(x)}"><b>${safe(x.title)}</b><small>${safe(x.sub)}</small></button>`).join('')||'<div class="note">Ничего не найдено</div>';$$('[data-search-i]').forEach(b=>b.onclick=()=>{closeSheet();items[Number(b.dataset.searchI)].go()})};
...
$('#historyTile').onclick=()=>openSheet(`<h3>Все операции</h3><div style="font-size:27px;font-weight:900;margin-bottom:10px">${rub(state.spend)}</div>${historyHtml()}<div class="note">История относится только к симулятору.</div>`);
